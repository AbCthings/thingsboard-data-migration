...
def getData(ip,port,user,password,deviceId,key,startTs,endTs):
    ...
    try:
        ...
        logging.error('Timeseries request to device '+deviceId+' failed.')
        sys.exit(1)
    ...
    logging.info('Timeseries request to device '+deviceId+' was successful.')
...
def sendData(ip,port,deviceToken,key,timeseries):
    ...
    try:
        ...
        logging.error('Timeseries upload to device '+deviceToken+' failed.')
        pass
    else:
        print("Data successfully published via MQTT.")

    logging.info('Timeseries upload to device '+deviceToken+' was successful.')
